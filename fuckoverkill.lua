...
if UI_Success and OrionLib then
    local success = pcall(function()
        ...
        -- RAGE TAB (Empty for now)
        local RageTab = Window:MakeTab({ Name = "Rage", Icon = "rbxassetid://4483345998", PremiumOnly = false })
...
if not UI_Worked then
    ...
    Tab2.Size = UDim2.new(1/4, 0, 1, 0)
    Tab2.Position = UDim2.new(1/4, 0, 0, 0)
    ...
    Tab3.Size = UDim2.new(1/4, 0, 1, 0)
    Tab3.Position = UDim2.new(2/4, 0, 0, 0)
    ...
    Tab4.Size = UDim2.new(1/4, 0, 1, 0)
    Tab4.Position = UDim2.new(3/4, 0, 0, 0)
    Tab4.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
    Tab4.TextColor3 = Color3.fromRGB(255, 255, 255)
    Tab4.Text = "Settings"
    Tab4.Font = Enum.Font.SourceSansBold
    Tab4.TextSize = isMobile and 14 or 14
    ...
    local function SwitchTab(tab, pageNum)
    ...
    Tab1.MouseButton1Click:Connect(function() SwitchTab(Tab1, 1) end)
    Tab2.MouseButton1Click:Connect(function() SwitchTab(Tab2, 2) end)
    Tab3.MouseButton1Click:Connect(function() SwitchTab(Tab3, 3) end)
    Tab4.MouseButton1Click:Connect(function() SwitchTab(Tab4, 4) end)
    ...
    y = 0
    y = MakeToggle(Page3, y, "Enabled", ESP_Enabled, function(v) ESP_Enabled = v end)
    y = MakeToggle(Page3, y, "Dead Check", Dead_Check_Enabled, function(v) Dead_Check_Enabled = v; if not v then ClearTable(Player_Dead) end end)
    y = MakeToggle(Page3, y, "Box", Box_Enabled, function(v) Box_Enabled = v end)
    y = MakeToggle(Page3, y, "Name", Name_Enabled, function(v) Name_Enabled = v end)
    y = MakeToggle(Page3, y, "Health Bar", Health_Enabled, function(v) Health_Enabled = v end)
    y = MakeToggle(Page3, y, "Distance", Distance_Enabled, function(v) Distance_Enabled = v end)
    y = MakeToggle(Page3, y, "Tracers", Tracer_Enabled, function(v) Tracer_Enabled = v end)
    y = MakeDropdown(Page3, y, "Tracer Origin", {"Bottom", "Middle", "Top"}, "Bottom", function(v) Tracer_Origin = v end)
    y = MakeToggle(Page3, y, "Head Dot", Head_Dot_Enabled, function(v) Head_Dot_Enabled = v end)

    y = 0
    y = MakeToggle(Page4, y, "Fallback UI", true, function() end)
    y = MakeDropdown(Page4, y, "UI Theme", {"Default Blue", "Red", "Green", "Purple", "Pure Black", "Pink"}, "Default Blue", function(v)
...
local function Get_Nearest_Part(player, part_name)
    ...
    if part_name == "Random" then
        local parts = {}
        for _, v in ipairs(char:GetChildren()) do if v:IsA("BasePart") then table_insert(parts, v) end
    ...
    for _, player in ipairs(Services.Players:GetPlayers()) do
        if player ~= Local_Player then
            ...
            if not skip then
                ...
                -- FIXED ESP OFFSET: Calculate box using distance (ignores camera pitch entirely)
                ...
                -- Stable height calculation based on distance (doesn't break when looking up/down)
                local height = (1000 / math.clamp(distance, 1, 1000)) * 4
                local width = height * 0.55
                
                -- Calculate center point between head and root
...
end)local function Set_Aim_Key(value)
    Aim_Key_Name = value
    if value == "MB1" then Aim_Key = Enum.UserInputType.MouseButton1 else Aim_Key = Enum.UserInputType.MouseButton2 end
end
