...
if not UI_Worked then
    ...
    -- DRASTIC MOBILE FIXES: Tiny box, Huge text
...
if not UI_Worked then
    ...
    -- MOBILE SIZING FIX: Smaller window, Bigger text, tighter rows
    local uiSizeX = isMobile and 240 or 350
    local uiSizeY = isMobile and 360 or 450
    local fontSize = isMobile and 17 or 13
    local rowHeight = isMobile and 30 or 28
    local btnSizeX = isMobile and 42 or 40
    local btnSizeY = isMobile and 22 or 20
    local titleHeight = isMobile and 30 or 40
    local tabHeight = isMobile and 26 or 35
    ...
    local ContentFrame = Instance.new("Frame")
    ContentFrame.Size = UDim2.new(1, -10, 1, -titleHeight - tabHeight - 15)
    ContentFrame.Position = UDim2.new(0, 5, 0, titleHeight + tabHeight + 5)
    ContentFrame.BackgroundTransparency = 1
    ContentFrame.ClipsDescendants = false
    ContentFrame.Parent = MainFrame
    ...
    Tab1.Text = "Aim"
    Tab1.Font = Enum.Font.SourceSansBold
    Tab1.TextSize = isMobile and 15 or 14
    Tab1.BorderSizePixel = 0
    Tab1.Parent = TabFrame
    ...
    Tab2.Text = "ESP"
    Tab2.Font = Enum.Font.SourceSansBold
    Tab2.TextSize = isMobile and 15 or 14
    Tab2.BorderSizePixel = 0
    Tab2.Parent = TabFrame
    ...
    Tab3.Text = "Settings"
    Tab3.Font = Enum.Font.SourceSansBold
    Tab3.TextSize = isMobile and 15 or 14
    Tab3.BorderSizePixel = 0
    Tab3.Parent = TabFrame
    ...
    local function MakeToggle(parent, y, text, default, callback)
        ...
        frame.Parent = parent
        local lbl = Instance.new("TextLabel")
        lbl.Size = UDim2.new(1, -btnSizeX - 10, 1, 0)
        ...
        local btn = Instance.new("TextButton")
        btn.Size = UDim2.new(0, btnSizeX, 0, btnSizeY)
        btn.Position = UDim2.new(1, -btnSizeX - 4, 0.5, -btnSizeY/2)
        if default then btn.BackgroundColor3 = Color3.fromRGB(0, 180, 0); btn.Text = "ON" else btn.BackgroundColor3 = Color3.fromRGB(80, 80, 80); btn.Text = "OFF" end
        btn.TextColor3 = Color3.new(1, 1, 1)
        btn.Font = Enum.Font.SourceSansBold
        btn.TextSize = isMobile and 13 or 10
        btn.BorderSizePixel = 0
        btn.Parent = frame
        btn.MouseButton1Click:Connect(function()
            default = not default
    ...
    local function MakeDropdown(parent, y, text, options, default, callback)
        ...
        btn.Parent = frame
        local listFrame = Instance.new("Frame")
        listFrame.Size = UDim2.new(1, -6, 0, #options * (rowHeight - 4))
        listFrame.Position = UDim2.new(0, 3, 0, -#options * (rowHeight - 4) - 2)
        listFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 60)
        listFrame.BorderSizePixel = 0
        listFrame.Visible = false
        listFrame.ZIndex = 10
...
-- ============ UI SETUP (SAFE) ============
-- Skip HTTP on mobile to prevent instant crashes
local UI_Success, OrionLib = false, nil
if not isMobile then
    UI_Success, OrionLib = pcall(function()
        return loadstring(game:HttpGet("https://raw.githubusercontent.com/shlexware/Orion/main/source"))()
...
-- ============ GAME LOGIC (100% ISOLATED SO IT CANNOT CRASH THE UI) ============
pcall(function()
    local replicated_first = game:GetService("ReplicatedFirst")
    local peuron = require(replicated_first:WaitForChild("neuron"))
    
    -- DOUBLE ISOLATION: If get_character crashes, it won't kill the menu
    task.spawn(function()
        while task.wait() do
            pcall(function()
                for _, v in game.Players:GetPlayers() do
                    if not v.Character then
                        v.Character = peuron:get_character(v)
                    end
                end
            end)
        end
