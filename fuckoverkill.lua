...
local ToggleMenu = function() end -- Will be defined properly later

local function Set_Aim_Key(value)
    Aim_Key_Name = value
    if value == "MB1" then Aim_Key = Enum.UserInputType.MouseButton1
    else Aim_Key = Enum.UserInputType.MouseButton2 end
end
...
if UI_Success and OrionLib then
    ...
    if success then
        UI_Worked = true
        ToggleMenu = function()
            pcall(function() OrionLib:ToggleUI() end)
        end
    end
end

-- ============ FALLBACK UI (ONLY RUNS IF ORION FAILED) ============
if not UI_Worked then
    ...
    local uiSizeX = isMobile and 300 or 350
    local uiSizeY = isMobile and 420 or 450
    local fontSize = isMobile and 15 or 13
    local dropFontSize = isMobile and 14 or 13
    local btnSizeX = isMobile and 45 or 40
    local btnSizeY = isMobile and 24 or 20
    local rowHeight = isMobile and 34 or 28
    ...
    Tab1.TextSize = isMobile and 16 or 14
    ...
    Tab2.TextSize = isMobile and 16 or 14
    ...
    Tab3.TextSize = isMobile and 16 or 14
    ...
    local Themes = {
        ...
        ["Orange"] = { Main = Color3.fromRGB(40, 30, 20), Primary = Color3.fromRGB(80, 50, 30), Secondary = Color3.fromRGB(120, 70, 40), ActiveTab = Color3.fromRGB(120, 70, 40), InactiveTab = Color3.fromRGB(50, 35, 25) },
        ["Cyan"] = { Main = Color3.fromRGB(20, 35, 40), Primary = Color3.fromRGB(30, 70, 80), Secondary = Color3.fromRGB(40, 100, 120), ActiveTab = Color3.fromRGB(40, 100, 120), InactiveTab = Color3.fromRGB(25, 45, 50) },
        ["Dark Blue"] = { Main = Color3.fromRGB(15, 15, 30), Primary = Color3.fromRGB(20, 20, 60), Secondary = Color3.fromRGB(30, 30, 90), ActiveTab = Color3.fromRGB(30, 30, 90), InactiveTab = Color3.fromRGB(10, 10, 40) }
    }
    ...
    local function MakeToggle(parent, y, text, default, callback)
        ...
        lbl.Size = UDim2.new(1, -btnSizeX - 15, 1, 0)
        ...
        btn.Position = UDim2.new(1, -btnSizeX - 5, 0.5, -btnSizeY/2)
    ...
    local function MakeDropdown(parent, y, text, options, default, callback)
        ...
        btn.TextSize = dropFontSize
        btn.BorderSizePixel = 0
        btn.ZIndex = 6
        btn.Parent = frame
        local listFrame = Instance.new("Frame")
        listFrame.Size = UDim2.new(1, -10, 0, #options * (rowHeight - 4))
        listFrame.Position = UDim2.new(0, 5, 0, -#options * (rowHeight - 4) - 2)
    ...
    y = MakeToggle(Page1, y, "Always On (Mobile)", false, function(v) Aim_Always_On = v end)
    ...
    y = MakeToggle(Page1, y, "Max Distance", false, function(v) Max_Distance_Enabled = v end)
    ...
    y = MakeDropdown(Page3, y, "UI Theme", {"Default Blue", "Red", "Green", "Purple", "Pure Black", "Pink", "Orange", "Cyan", "Dark Blue"}, "Default Blue", function(v)
        if Themes[v] then ApplyTheme(Themes[v]) end
    end)

    -- Set the toggle function for Fallback UI
    ToggleMenu = function()
        if MainFrame then MainFrame.Visible = not MainFrame.Visible end
    end
end

-- ============ MOBILE BUTTON (CREATED ONCE AT THE END) ============
if isMobile then
    ...
    mobileBtn.Position = UDim2.new(0, 10, 0.5, -35)
    ...
    mobileBtn.InputEnded:Connect(function(input)
        if dragging and input.UserInputState == Enum.UserInputState.End then
            dragging = false
            -- Only toggle if it was a tap (drag distance < 10 pixels)
...
local function Aim_At_Target(player)
    ...
    if isMobile then
        ...
        if look_vec:Dot(Camera.CFrame.LookVector) > 0.1 then
            local target_cframe = CFrame.new(cam_pos, cam_pos + look_vec)
            -- Fixed math: 0 smoothness = very fast lock (0.8), 99 = very slow (0.05)
...
if not UI_Worked then
    ...
    local uiSizeX = isMobile and 280 or 350
    local uiSizeY = isMobile and 380 or 450
...
-- DEFINE TOGGLE MENU FUNCTION FOR MOBILE BUTTON
ToggleMenu = function()
    if UI_Worked and OrionLib then
        pcall(function() OrionLib:ToggleUI() end)
    ...
    for _, p in ipairs(Services.Players:GetPlayers()) do
        if p ~= Local_Player and not Is_Same_Team(p) and not (Dead_Check_Enabled and Is_Player_Dead(p)) then
            ...
            if part then
                local skip_target = false
                if Max_Distance_Enabled then
                    if Get_Distance_From_Local(p) > Max_Distance then
                        skip_target = true
                    end
                end
                if not skip_target then
...
local function Aim_At_Target(player)
    ...
    -- MOBILE SPECIFIC AIMING (FIXES SPAZZING)
    if isMobile then
        local target_pos = part.Position
        local cam_pos = Camera.CFrame.Position
        local look_vec = (target_pos - cam_pos)
        
        -- Prevent flipping backwards wildly
        if look_vec:Dot(Camera.CFrame.LookVector) > 0 then
            local target_cframe = CFrame.new(cam_pos, cam_pos + look_vec)
            local s = math_clamp(1 - (math_clamp(Aim_Smoothness, 0, 99) / 100), 0.01, 1)
            Camera.CFrame = Camera.CFrame:Lerp(target_cframe, s)
        end
